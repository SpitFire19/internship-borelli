...
def compute_window(args):
    ...
    print(f"Worker started ({i+1}, {j+1})")
    ...
    for epoch in range(n_epochs):
        ...
        no_change = np.concatenate([before, no_change_after], axis=0)
        changed = np.concatenate([before, change_after], axis=0)
        ...
        dd_total["Energy-based"] += dd - n_samples // 2
...
if __name__ == "__main__":
    ...
    jobs = [(i, j) for i in range(n) for j in range(n) if i != j]

    with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
        results = executor.map(compute_window, jobs)

        for i, j, arl_total, dd_total in results:
